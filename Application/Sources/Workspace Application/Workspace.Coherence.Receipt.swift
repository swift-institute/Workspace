...
extension Workspace.Coherence {
    ...
    /// Workspace revision, the inventory it read, which selection was in
...
extension Workspace.Coherence.Receipt {
    ...
    /// same reasoning ``Workspace/Lint/Install/digestOf(_:)`` documents:
    /// the ecosystem publishes no SHA-2 implementation. The canonical text
    /// is written to a scratch file beside the checkout and removed
    ...
    public func digest(at root: Workspace.Root) throws(Workspace.Error) -> Swift.String {
        ...
        do throws(File.Path.Temporary.Error) {
            path = try File.Path.Temporary.sibling(
                ...
                prefix: ".workspace-coherence-receipt-",
                suffix: ".json"
            )
        } catch {
