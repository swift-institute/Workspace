...
extension Workspace.Composition.Clause.Test.Unit {
    ...
    func `all finds every package clause in source order`() {
        let clauses = Workspace.Composition.Clause.all(in: Self.manifest)
    ...
    func `url locates the clause by derived identity`() {
        let clause = Workspace.Composition.Clause.url(identity: "swift-beta", in: Self.manifest)
        #expect(clause?.declaredURL == "https://github.com/foo/swift-beta.git")
    }

    @Test
    func `url returns nil when no dependency has that identity`() {
        #expect(Workspace.Composition.Clause.url(identity: "swift-delta", in: Self.manifest) == nil)
    }
    ...
    func `identity derivation strips git, slash, and case`(
        ...
        #expect(Workspace.Composition.Clause.identity(ofURL: url) == expected)
    }

    @Test
    func `compose then restore is byte-identical`() {
        let original = Self.manifest
        let clause = Workspace.Composition.Clause.url(identity: "swift-alpha", in: original)!
        ...
        let composedClause = Workspace.Composition.Clause.all(in: composed).first { $0.text == planned }!
...
extension Workspace.Composition.Clause.Test.`Edge Case` {
    ...
    func `a parenthesis inside a quoted path does not truncate the clause`() {
        ...
        let clause = Workspace.Composition.Clause.all(in: source).first
    ...
    func `unbalanced parentheses stop enumeration without trapping`() {
        ...
        #expect(Workspace.Composition.Clause.all(in: source).isEmpty)
    }

    @Test
    func `a url clause reports no declared path and a path clause no declared url`() {
        let url = Workspace.Composition.Clause.all(in: ".package(url: \"https://e.com/x.git\", branch: \"main\")").first
        #expect(url?.declaredPath == nil)
        let path = Workspace.Composition.Clause.all(in: ".package(path: \"/x\")").first
