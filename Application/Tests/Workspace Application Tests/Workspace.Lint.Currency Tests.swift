...
struct `Workspace Lint Currency Tests` {
    ...
    func `a current install reports nothing`() throws {
        ...
        #expect(Workspace.Lint.currency(of: manifest, against: Self.matching).isEmpty)
    }
    ...
    func `a moved rule pack is named, with the remedy`() throws {
        ...
        let findings = Workspace.Lint.currency(of: manifest, against: heads)
    ...
    func `an input absent from the manifest refuses`() throws {
        ...
        let findings = Workspace.Lint.currency(of: manifest, against: Self.matching)
        #expect(findings.count == 3)
        #expect(
            findings[1]
